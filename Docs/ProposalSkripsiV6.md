...
$$\mathcal{B}_t = \{(\mathbf{f}_j, y_j^{\text{corrected}}) \mid j \in \text{corrections since } t_{\text{last\_retrain}}\} \tag{6}$$

*Trigger Retraining.* Retraining model diinisiasi ketika buffer mencapai ambang akumulasi minimum:

$$|\mathcal{B}_t| \geq n_{\text{min}} \quad \text{di mana } n_{\text{min}} = 50 \tag{7}$$
